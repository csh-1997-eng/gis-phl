...
def main() -> int:
    ...
    pd.DataFrame([{"row_count": int(len(df)), "geo_count": int(df["geography_entity_id"].nunique()), "states": ",".join(a.region_states), "min_period": str(df["period_dt"].min().date()), "max_period": str(df["period_dt"].max().date()), "min_history_months": a.min_history_months}]).to_csv(a.artifacts_dir / "dataset_summary.csv", index=False)

    (df.groupby("state_name", as_index=False).agg(zip_geo_count=("geography_entity_id", "nunique"), row_count=("entity_id", "size")).sort_values("zip_geo_count", ascending=False)).to_csv(a.artifacts_dir / "zip_panel_state_summary.csv", index=False)

    metrics = {
        "scope": {"region_type": "zip", "states": a.region_states, "min_history_months": a.min_history_months},
